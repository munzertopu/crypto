  ...
  return (
    <div className="px-2 sm:px-4 lg:px-6 py-2 lg:py-4">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-4 sm:mb-6">
        {/* Title */}
        <h1 className={`text-xl lg:text-xl font-medium`} style={{color: `${isDarkMode ? '#E1E3E5' : '#0E201E'}`}}>Dashboard</h1>
        ...
        >
          <Datepicker
            ...
            primaryColor='green'
            inputClassName={`w-full rounded-md bg-transparent mr-8 focus:outline-none text-sm sm:text-base ${isDarkMode ? 'placeholder:text-white text-white' : 'placeholder:text-gray-800 text-gray-800'}`}
            containerClassName="relative pr-6"
            toggleClassName={`absolute rounded-r-lg px-0 right-0 top-0 h-full ${isDarkMode ? 'text-white' : 'text-gray-800'}`}
      ...
      <div className={`${isDarkMode ? 'border-gray-700' : 'border-gray-200'}`}>
        <Tabs>
          <Tabs.List className={`my-2 lg:my-2 space-x-1 ${isDarkMode ? "bg-[#2F3232]" : "bg-[#F3F5F7]"}`}>
            {tabs.map((tab) => (
              <Tabs.Trigger 
                key={tab}
                value={tab}
