...
const SettingsPage: React.FC<SettingsPageProps> = ({
  ...
  const settingsTabs: SettingsTab[] = [
    { id: 'portfolio', name: 'Portfolio', icon: PortfolioIcon, component: PortfolioTab },
    { id: 'tax', name: 'Tax', icon: TaxIcon, component: TaxTab },
    { id: 'formInfo', name: 'Form Info', icon: FormInfoIcon, component: FormInfoTab },
    { id: 'costBasis', name: 'Cost Basis', icon: CostBasisIcon, component: CostBasisTab },
    { id: 'rules', name: 'Rules', icon: RulesIcon, component: RulesTab },
    { id: 'customPrice', name: 'Custom Price', icon: CustomPriceIcon, component: CustomPriceTab },
  ...
  return (
    <div className={`${isDarkMode ? 'bg-[#0E201E] text-white' : 'bg-white'}`}>
      ...
      <div className="flex ml-10">
        {/* Left Sidebar */}
        <div className="py-4">
          <Typography variant="h4" className={`text-xl font-semibold mb-8 text-left ${isDarkMode ? 'text-white' : 'text-[#0E201E]'}`}>
            Settings
          </Typography>
          ...
          <nav className="space-y-2">
            {settingsTabs.map((tab) => (
              <button
                ...
                className={`w-full flex items-center space-x-4 px-4 py-2 rounded-lg text-left ${
                  activeTab === tab.id
                    ? `${isDarkMode ? 'bg-gray-700 text-white border border-gray-600' : 'text-[#0E201E] bg-[#F3F5F7]'}`
                    : `${isDarkMode ? 'text-gray-300 hover:bg-gray-700 hover:text-white' : 'text-[#0E201E]  hover:bg-[#F3F5F7]'}`
                }`}
              >
                <tab.icon 
                  className={`w-4 h-4 ${activeTab === tab.id ? 'text-[#7C7C7C]' : 'text-[#7C7C7C]'}`}
                />
        ...
        <div className="flex-1 p-8">
          <Card className={`border-transparent shadow-transparent ${isDarkMode ? 'bg-gray-800' : 'bg-white'}`}>
