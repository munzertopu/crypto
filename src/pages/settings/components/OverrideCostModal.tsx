...
interface OverrideCostModalProps {
  ...
  onClose: () => void;
  onSubmit: (
    selectedTaxYears: string[],
    selectedCostBasisMethod: string
  ) => void;
}
...
const OverrideCostModal: React.FC<OverrideCostModalProps> = ({
  isOpen,
  onClose,
  onSubmit,
}) => {
  const [selectedTaxYears, setSelectedTaxYears] = useState<string[]>([]);
  const [selectedCostBasisMethod, setSelectedCostBasisMethod] =
    useState<string>("");
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      if (
        taxYearRef.current &&
        !taxYearRef.current.contains(event.target as Node) &&
      ...
      if (
        ...
        !costBasisRef.current.contains(event.target as Node) &&
        screenSize.width >= 768
      ) {
  ...
  const isSubmitDisabled =
    selectedTaxYears.length === 0 || !selectedCostBasisMethod;
  ...
  return (
    <div>
      <div className="hidden md:fixed inset-0 bg-black bg-opacity-50 md:flex items-center justify-center z-50">
        <div
          className={`relative max-w-md w-full p-8 mx-4 rounded-lg shadow-lg bg-white text-gray-900
      dark:bg-gray-800 dark:text-gray-250`}
        >
          ...
          <div className="flex text-left items-start justify-between">
            <div>
              <h2
                className={`text-h5 font-bold text-[#191919] dark:text-gray-300`}
              >
                Override cost basis method
              </h2>
              <p
                className={`text-base mt-1.5 text-[#191919] dark:text-gray-300`}
              >
                Apply a different cost basis method for selected tax years,
                overriding your default setting.
              </p>
            </div>
          ...
          <div className="text-left space-y-3 mt-6">
            ...
            <div className="space-y-2">
              ...
              <div className="relative" ref={taxYearRef}>
                <button
                  onClick={() =>
                    setIsTaxYearDropdownOpen(!isTaxYearDropdownOpen)
                  }
                ...
                >
                  <span
                    className={
                      selectedTaxYears.length > 0 ? "" : "text-gray-500"
                    }
                  >
                    {getDisplayText()}
                  </span>
                ...
                {isTaxYearDropdownOpen && (
                  <div
                    className={`absolute z-10 w-full mt-1 border border-gray-300 rounded-lg max-h-48 overflow-y-auto ${"bg-white dark:bg-gray-700 dark:border-gray-600"}`}
                  >
                    {taxYears.map((year) => (
                      <button
                        key={year.value}
                        onClick={() => handleTaxYearToggle(year.value)}
                        className={`w-full px-3 py-2 text-left hover:bg-gray-50 flex items-center ${"text-gray-900 dark:hover:bg-gray-600 dark:text-gray-250"}`}
                      >
                        {/* Checkbox */}
                        <div
            ...
            <div className="space-y-2">
              ...
              <div className="relative" ref={costBasisRef}>
                <button
                  onClick={() =>
                    setIsCostBasisDropdownOpen(!isCostBasisDropdownOpen)
                  }
                  className={`w-full flex items-center justify-between px-3 py-2 border border-[#E1E3E5] rounded-lg focus:outline-none ${"bg-white text-gray-900 dark:bg-gray-700 dark:border-gray-600 dark:text-gray-250"}`}
                >
                  <span
                    className={selectedCostBasisMethod ? "" : "text-gray-500"}
                  >
          ...
          <div className="flex justify-between mt-6">
            <button
              ...
              className={`px-4 py-3 font-medium ${"text-[#7C7C7C] dark:text-gray-300 dark:hover:text-white"}`}
            >
              Cancel
            </button>
            <button
      ...
      <MobileDrawer
        header="Override cost basis method"
        isOpen={isOpen}
        onClose={onClose}
        ...
        <div className="text-left space-y-3 mt-6">
          ...
          <div className="space-y-2">
            ...
            <div className="relative">
              ...
              >
                <span
                  className={selectedTaxYears.length > 0 ? "" : "text-gray-500"}
                >
                  {getDisplayText()}
                </span>
                <FontAwesomeIcon
      ...
      <MobileDrawer
        isOpen={isTaxYearDropdownOpen}
        header="Select Tax year"
        onClose={() => setIsTaxYearDropdownOpen(false)}
        hideCloseIcon={true}
        disableRightButton={selectedTaxYears.length === 0}
        className="z-51"
      >
        {taxYears.map((year) => (
          <button
            ...
            onClick={(e) => {
              e.stopPropagation();
              handleTaxYearToggle(year.value);
            }}
            className={`w-full px-3 py-2 text-left hover:bg-gray-50 flex items-center ${"text-gray-900 dark:hover:bg-gray-600 dark:text-gray-250"}`}
          >
            {/* Checkbox */}
      ...
      <MobileDrawer
        isOpen={isCostBasisDropdownOpen}
        header="Select Cost Basis Method"
        onClose={() => setIsCostBasisDropdownOpen(false)}
        hideCloseIcon={true}
        className="z-51"
        disableRightButton={selectedCostBasisMethod === ""}
      >
