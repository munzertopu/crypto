...
const CryptoDetailsPage: React.FC<CryptoDetailsPageProps> = () => {
  ...
  const getCryptoDetails = (symbol: string) => {
    const cryptoMap: {
      [key: string]: { name: string; symbol: string; logo: string };
    } = {
      ETH: {
        name: "Ethereum",
        symbol: "ETH",
  ...
  const [activeTableTab, setActiveTableTab] = useState<
    "allocations" | "taxlots" | "insightssummary"
  >("allocations");
  ...
  return (
    ...
    >
      ...
      <div className="px-4 md:px-10 sm:px-6 md:pt-5 md:pb-2 w-full mt-6 md:mt-0">
        {/* Breadcrumb */}
        <nav className="flex mb-8" aria-label="Breadcrumb">
          <ol className="inline-flex items-center space-x-1 md:space-x-3">
            <li className="inline-flex items-center">
              <Link
                to="/dashboard"
                className="inline-flex items-center text-base font-normal text-[#8C8E90] dark:text-gray-400 dark:hover:text-white"
              >
                Dashboard
              </Link>
            </li>
            ...
            <Tabs.List className="bg-gray-100 dark:bg-[#2F3232] w-full md:w-max justify-between">
              {tableTabs.map((tab) => (
                <Tabs.Trigger
                  ...
                  onClick={() =>
                    setActiveTableTab(
                      tab.toLowerCase().replace(/\s+/g, "") as any
                    )
                  }
        ...
        {/* Allocations Table */}
        {activeTableTab === "allocations" && <AllocationTable />}

        {/* Tax Lots Tab */}
        {activeTableTab === "taxlots" && <TaxLotTable />}

        {/* Insights Summary Tab */}
        {activeTableTab === "insightssummary" && <InsightsSummary />}
      </div>
    </div>
  );
};
