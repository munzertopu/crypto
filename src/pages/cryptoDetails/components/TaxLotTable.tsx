...
const TaxLotTable: React.FC<TaxLotTableProps> = ({ data = [] }) => {
            ...
            <table className="w-full min-w-max table-auto text-left">
              ...
              <tbody className="bg-white dark:bg-[#0E201E] divide-gray-200 dark:divide-[#2F3232]">
                {tableData.map((item) => (
                  <tr
                    key={item.id}
                    className="hover:bg-gray-50 dark:hover:bg-[#2F3232]"
                  >
                    <td className="px-5 py-3 whitespace-nowrap">
                      <div className="flex items-center">
                        <Avatar
                          src={`/crypto/${item.acquisitionAccount.logo}`}
                          alt={item.acquisitionAccount.name}
                          size="sm"
                          className="mr-1.5"
                        />
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center">
                        <Avatar
                          src={`/crypto/${item.currentAccount.logo}`}
                          alt={item.currentAccount.name}
                          size="sm"
                          className="mr-1.5"
                        />
                        <span className="text-base text-[#0E201E] dark:text-gray-250">
                    ...
                    <td className="px-5 py-3 whitespace-nowrap">
                      <div className="text-base text-[#0E201E] dark:text-gray-250">
                        {item.currentPrice.current}
                      </div>
                      <div className="text-sm text-gray-500 dark:text-[#A1A3A5]">
                        {item.currentPrice.original}
                      </div>
                    </td>
                    <td className="px-5 py-3 whitespace-nowrap">
                      <div className="text-base text-[#0E201E] dark:text-gray-250">
                      ...
                      <div className="text-sm text-gray-600 dark:text-[#8C8E90]">
                    ...
                    <td className="px-5 py-3 whitespace-nowrap">
                      <span
                        className={`text-base font-medium ${
                          item.unrealizedGL.type === "positive"
                            ? "text-green-600"
                            : "text-red-600"
                        }`}
                      >
                ...
                <div className="px-4 pb-4 space-y-3 border-t border-gray-200 dark:border-[#4D5050] pt-3">
                  ...
                  <div className="flex justify-between items-start">
                    ...
                    <div className="text-right">
                      ...
                      <div className="text-xs text-gray-600 dark:text-gray-250">
                        {item.currentPrice.original}
                      </div>
                    </div>
                  </div>
