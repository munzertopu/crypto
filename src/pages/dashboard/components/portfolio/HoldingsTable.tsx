...
const HoldingsTable: React.FC<HoldingsTableProps> = ({
  ...
  const TABLE_HEAD = ["Asset", "Balance", "ROI", "Cost", "Market Value", "Statistic for 24h"];
  ...
  const TrendChart: React.FC<{ trend: 'up' | 'down'; data?: number[] }> = ({ trend, data = [] }) => {
    ...
    return (
      <div className="flex items-center justify-between w-full">
        ...
        <svg className="w-6 h-6 mr-4 flex-shrink-0" viewBox="0 0 16 16" fill="none" >
          <circle cx="8" cy="8" r="7" stroke="#9CA3AF" strokeWidth="1.5" fill="none"/>
          <path d="M5 8L7 10L11 6" stroke="#9CA3AF" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
        </svg>
      </div>
    );
  };
  ...
  return (
    <div className="px-8 mb-6">
        <div className="py-4">
          <h3 className={`text-2xl font-semibold ${isDarkMode ? "text-white" : "text-[#0E201E]"} text-left`}>Holdings</h3>
        </div>
        <Card className={`h-full w-full border-transparent bg-transprent`}>
          <CardBody className="px-0 rounded-lg">
            <table className="w-full min-w-max table-auto text-left">
              <thead className={`${isDarkMode ? "bg-[#2F3232]" : "bg-[#F3F5F7]"}`}>
                <tr className=''>
                  {TABLE_HEAD.map((head, index) => (
                    <th
                      key={head}
                      className={`cursor-pointer p-6 ${index === 0 ? 'rounded-l-xl' : ''} ${index === TABLE_HEAD.length - 1 ? 'rounded-r-md' : ''}`}
                    >
                      <Typography
                        variant="small"
                        className={`flex items-center justify-between gap-2 font-normal text-xl leading-none ${isDarkMode ? "text-[#B6B8BA]" : "text-[#666868]"}`}
                      >
                        {head}{" "}
              ...
              <tbody>
                {data.map(
                  ({ id, name, symbol, logo, balance, roi, roiChange, cost, marketValue, trend24h, trendData }, index) => {
                      ...
                      <tr key={id}>
                        <td className='p-5'>
                          <div className="flex items-center gap-3">
                            ...
                            <div className="flex flex-col">
                              <Typography
                                variant="small"
                                className={`font-medium text-xl ${onCryptoClick ? 'cursor-pointer' : ''} ${isDarkMode ? "text-[#F3F5F7]" : "text-[#0E201E]"}`}
                                onClick={() => onCryptoClick && onCryptoClick(symbol)}
                                role={onCryptoClick ? "button" : undefined}
                                aria-label={onCryptoClick ? `View details for ${name} (${symbol})` : undefined}
                              >
                                {name}
                              </Typography>
                              <Typography
                        ...
                        <td>
                          <div className="flex flex-col">
                            <Typography
                              variant="small"
                              className={`font-normal text-lg ${isDarkMode ? "text-[#F3F5F7]" : "text-[#0E201E]"}`}
                            >
                              {balance}
                            </Typography>
                          </div>
