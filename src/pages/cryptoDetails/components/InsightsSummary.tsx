    ...
    <div className={`${isDarkMode ? 'bg-gray-800' : 'bg-white'}`}>
      <div className="pt-6 pb-4">
        <div className="flex items-center gap-2 mb-4">
          <h3 className="text-lg font-semibold text-left">Insights Summary</h3>
        ...
        {/* Dismissible Banner */}
        {showBanner && (
          <div className={`mb-4 p-3 rounded-lg border border-blue-200 ${isDarkMode ? 'bg-gray-700' : 'bg-white'}`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {insightCards.map((card) => (
            <div 
               key={card.id}
               className={`p-4`}
               style={{ backgroundColor: '#F4FAEB' }}
             >
               <div className="flex flex-col items-left text-left">
                 <div className="mb-3">
                   <div 
                    className="w-10 h-10 rounded-full flex items-center justify-center"
                    style={{ backgroundColor: isDarkMode ? '#90C853' : '#90C853' }}
                  >
                     <FontAwesomeIcon 
                       icon={card.icon} 
                       className="text-white text-sm" 
                       style={{ color: isDarkMode ? '#1B5E20' : '#E0F2F1' }}
                     />
                   </div>
                 </div>
