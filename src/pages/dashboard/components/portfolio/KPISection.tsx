...
import { faDollarSign, faChartLine, faClock, faChartBar, faCheck } from '@fortawesome/free-solid-svg-icons';
...
const KPICard: React.FC<KPICardProps> = ({ title, value, change, isPositive, icon, svgIcon, isDarkMode, centered = false }) => (
      ...
      <div className="flex items-center space-x-4">
        <div className="w-6 h-6 opacity-80 flex items-center justify-center">
          {svgIcon ? (
            ...
            <FontAwesomeIcon icon={icon} className={`${isDarkMode ? "text-[#B6B8BA]" : "text-[#0E201E]"} text-md`} />
          )}
        </div>
        <div className="flex items-center space-x-1">
          <span className={`text-xl opacity-80 font-medium ${isDarkMode ? "text-[#B6B8BA]" : "text-[#0E201E]"}`}>{title}</span>
        </div>
      </div>
      <div className="flex items-center space-x-2">
        <span className={`text-3xl font-bold ${isDarkMode ? "text-[#B6B8BA]" : "text-[#0E201E]"}`}>{value}</span>
        <span className={`text-lg font-medium ${isPositive ? 'text-[#5F9339]' : 'text-[#D8382C]'}`}>
          {change}
        </span>
      </div>
    </div>
  ...
  return (
    <div className="px-8 mb-6 mt-12">
      <div className="flex items-center justify-between mb-4">
        <h3 className={`text-3xl font-semibold ${isDarkMode ? "text-[#E1E3E5]" : "text-[#0E201E]"}`}>KPI</h3>
        <div className="relative pr-8" ref={dropdownRef}>
           {/* Add KPI Button with Dropdown */}
           <button
             onClick={handleDropdownToggle}
             className={`p-4 ${isDarkMode ? "text-[#B6B8BA] border-[#4D5050]" : "text-[#7C7C7C] border-[#E1E3E5]"} rounded-2xl bg-transparent flex items-center justify-center border`}
             aria-label="Add KPI"
           ...
           {isDropdownOpen && (
             <div 
               className={`absolute right-0 mt-2 w-64 border rounded-lg shadow-lg py-2 z-50 ${isDarkMode ? "bg-[#0E201E] border-[#4D5050]" : "bg-white border-[#E1E3E5]"}`}
               role="menu"
               aria-label="KPI selection menu"
             >
               <div className={`px-4 py-2 border-b ${isDarkMode ? "border-[#4D5050]" : "border-[#E1E3E5]"}`}>
                 <Typography variant="small" className={`font-semibold ${isDarkMode ? "text-[#F3F5F7]" : "text-[#0E201E]"}`}>
               ...
               <div className="py-2">
                 {Object.entries(selectedKPIs).map(([key, isSelected]) => {
                   ...
                   return (
                     <div 
                       key={key} 
                       className={`flex items-center px-4 py-2 ${isDarkMode ? "hover:bg-[#2F3232]" : "hover:bg-[#F3F5F7]"} cursor-pointer`}
                       onClick={() => handleKPIToggle(key)}
                       role="menuitemcheckbox"
                       aria-checked={isSelected}
                       aria-label={`Toggle ${kpiLabels[key]} KPI display`}
                     >
                       <div className={`w-5 h-5  border-2 rounded flex items-center justify-center mr-2 transition-colors border-[#90C853] ${
                         isSelected 
                           ? `${isDarkMode ? "bg-[#5F9339]" : "bg-white"}`
                           : `${isDarkMode ? "" : ""}`
                       }`}>
                         {isSelected && (
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="size-6 text-[#4f801cff]">
                              <path strokeLinecap="round" strokeLinejoin="round" d="m4.5 12.75 6 6 9-13.5" />
                            </svg>

                         )}
                       </div>
                       <Typography variant="small" className={`${isDarkMode ? "text-[#F3F5F7]" : "text-[#0E201E]"}`}>
                         {kpiLabels[key]}
                       </Typography>
                     </div>
                   );
               ...
               {/* Action Buttons */}
               <div className={`flex justify-between px-4 py-2 border-t ${isDarkMode ? "border-[#4D5050]" : "border-[#E1E3E5]"}`}>
                 <button
                   onClick={handleCancel}
                   className={`px-3 py-1 text-sm font-medium rounded-md transition-colors ${isDarkMode ? "text-[#B6B8BA]" : "text-[#7C7C7C]"}`}
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        ...
        {/* Vertical Separator */}
        <div className={`hidden lg:block w-px ${isDarkMode ? "bg-[#4D5050]" : "bg-[#E1E3E5]"}`}></div>
