...
const TransactionPage: React.FC<TransactionPageProps> = ({ onLogout }) => {
  ...
  const filteredTransactions = mockTransactions.filter(transaction => {
    const matchesSearch = transaction.wallet.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
                         transaction.wallet.symbol.toLowerCase().includes(searchTerm.toLowerCase()) ||
    ...
    <div className={`min-h-screen ${isDarkMode ? 'bg-[#0E201E] text-white' : 'bg-white text-gray-900'}`}>
      ...
      <div className="mx-auto px-4 sm:px-6 lg:px-8 mt-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between px-2 lg:pl-8">
          <div>
            <h1 className="text-3xl font-bold mb-2 text-left">Transactions</h1>
          </div>
          <div className="flex items-center space-x-3 mt-4 sm:mt-0">
            <button className={`text-xl font-medium px-6 py-3 rounded-2xl bg-[#90C853] text-[#0E201E]`} aria-label="Add new transaction">
              Add Transaction
            </button>
          </div>
                 </div>
