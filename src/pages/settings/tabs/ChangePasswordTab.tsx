...
import { faCheckCircle, faXmarkCircle, faEye, faEyeSlash } from "@fortawesome/free-solid-svg-icons";
...
const ChangePasswordTab: React.FC<ChangePasswordTabProps> = ({
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            const errorMsg = error?.message || error?.toString() || "Failed to change password. Please try again.";
                    ...
                    <div>
                        ...
                        {newPassword && (
                            <div className="mt-3 space-y-1 text-xs">
                                <div className={`flex items-center ${passwordValidation.hasMinLength ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>
                                    <FontAwesomeIcon
                                        icon={passwordValidation.hasMinLength ? faCheckCircle : faXmarkCircle}
                                        className="w-3 h-3 mr-2"
                                    />
                                    At least 8 characters
                                </div>
                                <div className={`flex items-center ${passwordValidation.hasLowerCase ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>
                                    <FontAwesomeIcon
                                        icon={passwordValidation.hasLowerCase ? faCheckCircle : faXmarkCircle}
                                        className="w-3 h-3 mr-2"
                                    />
                                    At least one lowercase letter
                                </div>
                                <div className={`flex items-center ${passwordValidation.hasUpperCase ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>
                                    <FontAwesomeIcon
                                        icon={passwordValidation.hasUpperCase ? faCheckCircle : faXmarkCircle}
                                        className="w-3 h-3 mr-2"
                                    />
                                    At least one uppercase letter
                                </div>
                                <div className={`flex items-center ${passwordValidation.hasNumber ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>
                                    <FontAwesomeIcon
                                        icon={passwordValidation.hasNumber ? faCheckCircle : faXmarkCircle}
                                        className="w-3 h-3 mr-2"
                                    />
                                    At least one number
                                </div>
                                <div className={`flex items-center ${passwordValidation.hasSpecialChar ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>
                                    <FontAwesomeIcon
                                        icon={passwordValidation.hasSpecialChar ? faCheckCircle : faXmarkCircle}
                                        className="w-3 h-3 mr-2"
                                    />
