...
const Filters: React.FC<FiltersProps> = ({ 
  ...
  const [showEstimatedValueDropdown, setShowEstimatedValueDropdown] = useState(false);
      ...
      <div className="flex items-center">
        ...
        <div className="w-1/4 pr-4">
          <div className="relative">
            <FontAwesomeIcon 
              icon={faSearch} 
              className={`absolute left-3 top-1/2 transform text-xl -translate-y-1/2 ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}
            />
            <input
              type="text"
              placeholder="Search"
        ...
        <div className="w-2/4 flex gap-2">
             ...
             />
            <div className={`flex items-center space-x-2 rounded-lg border p-0 shadow-sm ${
            ...
            <div className="flex-1 relative">
               ...
               <FontAwesomeIcon 
                 icon={faChevronDown} 
                 className={`text-xs transition-transform ${showGainDropdown ? 'rotate-180' : ''}`}
               />
             </div>
        ...
        {/* Active Filter Tags */}
        {(fromValue !== '0' || toValue !== '0' || selectedDateRange || selectedGain !== 'Highest gain') && (
          <div className="mt-4 py-2">
            <div className="flex flex-wrap gap-2">
                ...
                <div className="flex items-center gap-2 px-3 py-1 bg-gray-200 rounded-full">
                  <span className="text-md font-semibold text-black-800">
                    {selectedDateRange.startDate && selectedDateRange.endDate 
                      ? `${new Date(selectedDateRange.startDate).toLocaleDateString('en-GB')} - ${new Date(selectedDateRange.endDate).toLocaleDateString('en-GB')}`
                      : selectedDateRange.startDate
                      ? new Date(selectedDateRange.startDate).toLocaleDateString('en-GB')
                      : 'Date selected'
                    }
                  </span>
                  <button
              ...
              {selectedGain !== 'Highest gain' && (
                <div className="flex items-center gap-2 px-3 py-1 bg-gray-200 rounded-full">
                  <span className="text-md font-semibold text-black">{selectedGain}</span>
                  <button
